...
def main(event=None):
    ...
    y = None
    a1 = None
    caption = "38.213 Table 13-15, pattern 3, {120, 120} kHz<br>illustrated with first half frame"
...
def mask_ssb(ssb_symb_row):
    i_lst = [4, 8, 16, 20]
    n_lst = [0, 1, 2, 3] + [5, 6, 7, 8] + [10, 11, 12, 13] + [15, 16, 17, 18]
    factor = 28
...
def mask_c0_symbol(ssb_symb_row, c0_symb_row):
    ...
    for i, s in enumerate(ssb_symb_row):
        if isinstance(s, str) and "ssb" in s:
            ...
            first = [4, 8, 2, 6][i_ssb % 4]
            i_symb0 = i - c0_symb_row[i] * 1 + first * 1
