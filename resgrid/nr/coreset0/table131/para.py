...
t.desc = ""
t.spec  = 'for frequency bands with minimum channel bandwidth 5 MHz or 10 MHz'
...
class Config(ConfigBase):
    ...
    def nrbs_change(self, event=None):
        m = self.mvalue
        if m == 24:
            self.set_range('offset', [0, 2, 4])
        if m == 48:
            self.set_range('offset', [12, 16])
        if m == 96:
            self.set_range('offset', [38])
