...
def main(event=None):
    ...
    ssb = {sc * 8 + o: (sc//12, sc, o) for sc in range(240)[::-1] for o in range(8)[::-1]}
    #
    kssb = {}
    if cfg.kssb.value == 0:
        kssb[4] = (0, 1)
    ...
    for i in  range(top, bottom - 1, -1):
        ...
        if i in coreset0:
            ...
            if rb in overlap_rbs:
                ...
                #     if half == 0 and sc == max(nonoverlap_subcarriers):
                #         row[3] = "  "
                #         row[4] = f"{min(nonoverlap_subcarriers)}..{max(nonoverlap_subcarriers)}"
                #         row[5] = rb
                #         class_row[3] = f"coreset0rb nonoverlap_subcarriers_right"
                #         class_row[4] = "coreset0rb nonoverlap_subcarriers_left"
    ...
    y = None
    caption = f"38.213 Table 13-9: RBs={cfg.nrbs.value}, Offset={cfg.offset.value}"
